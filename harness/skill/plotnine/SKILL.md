...
2. CHOOSE THE FORM       map the QUESTION to ONE geom family (references/geoms.md):
                         relationship → point · ranking/amount → bar/col · trend → line ·
                         distribution → histogram/density/boxplot · part-of-whole → bar
...
4. SCALES & LABELS       axis transforms/limits · real title + axis labels WITH UNITS ·
                         legend title · number/date formatting (references/api.md, geoms.md)
5. SMALL COLOR / THEME   fixed checklist: base theme · panel background · gridlines ·
                         text sizes · legend position · figure size + dpi (references/small_color.md)
...
- **Before you pick the geom** (Step 2): open `geoms.md`, find the row for the user's
  *question type*, and use the geom it names — plus its knobs (bar vs col, jitter/alpha
  for overplotting, bins for histograms). Do not free-associate a chart type.
...
- **Figure size + dpi.** Every plot saves at the house figure size and dpi (in
  `references/small_color.md`) so plots look consistent side by side. Never ship a
  default-size, low-dpi render.
...
- **Overplotting.** Many overlapping points → alpha and/or `geom_jitter`; a dense scatter
  may want `geom_bin2d`/`geom_density_2d`. `geoms.md` has the rule.
- **`fmt`/scales for numbers & dates.** Currency, percent, thousands, and date axes need
  the right `scale_*_continuous(labels=...)` / `scale_x_date(...)` — never ship raw
  `1e6`-style tick labels.

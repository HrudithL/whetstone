...
- **Conflict:** `capture` surfaces **cross-polarity** contradictions as its `conflict` status, resolved with the user via `revise`. A conflict is flagged when a new entry is highly similar to an existing opposite-polarity entry **and** one side *forbids* what the other *affirms* — decided by a **prohibition heuristic** on the issue text (`never`/`don't`/`avoid`/…). An aligned `Always …` mandate, or an aligned *negative* learning ("prefer avoiding X" next to "never X"), is therefore **not** flagged. Both directions are covered: new learning vs. existing issue (→ `remove`/`demote` the issue) and new issue vs. existing learning (→ `weaken`/`remove`/`promote` the learning), so `recall` can't later return both "do X" and "never do X". **`LEARNINGS`↔`LEARNINGS` contradictions are still *not* comprehensively auto-detected** — embeddings can't reliably separate a contradiction ("left-align") from a near-duplicate ("right-align") over the general case — so beyond the narrow check below, they are handled when the user explicitly routes the correction through `revise`/`weaken`. Detection is best-effort over the same-scope embedding neighbourhood already scanned for dedup — no separate subsystem, and a **heuristic** (prose polarity isn't perfectly recoverable from embeddings).
  - **§M7c — a narrow, additional same-polarity check (SPIKE, config-gated: `same_polarity_contradiction_check`).** On top of the embedding-similarity dedup match, `capture` runs a small, hand-picked, **explicitly non-exhaustive** antonym/negation lexicon (`_ANTONYM_PAIRS`/`_NEGATION_ASYMMETRY` in `server.py`: alignment, color temperature, spacing, ordering, orientation, position, magnitude, width, plus a handful of negation markers) over the candidate's and the existing entry's combined title+body. It flags an **asymmetry** — one side says "left", the other "right"; one side is negated ("never"/"avoid"/"don't"/"refrain"), the other isn't — and returns `possible_contradiction` (the existing entry's id, the candidate's body, and a short explanatory note) **instead of reinforcing**. Nothing is written in either case; it is a signal only, the same-polarity sibling of the `conflict` status above. A pair that mentions **both** sides of a lexicon entry (e.g. "left and right padding" in both texts), or is negated on **both** sides, is symmetric and is deliberately **not** flagged — the point is a flip, not shared vocabulary. This is validated by a small hand-labeled set (`harness/calibration/labels.yaml`'s `same_polarity_contradiction` section, scored by `harness/calibrate.py`'s `calibrate_same_polarity_contradiction`): **1.0 precision / 1.0 recall** on that labeled set (6 contradiction pairs, 5 duplicate pairs including deliberately tricky same-lexicon-word near-misses), on both the hashing and sentence-transformers backends. That labeled set is, by construction, matched against the lexicon it tests, so it alone is not independent evidence of real-world precision — manual adversarial spot-checks against realistic (non-hand-picked) paraphrase pairs during this slice's build is why the negation word list is deliberately **narrower** than the cross-polarity `_PROHIBITION` regex it was inspired by (that broader list's common filler words — bare "no"/"not"/"without" — produced false positives on ordinary phrasing like "no need for" or "not with a leading minus"; narrowing to `never`/`avoid`/`don't`/`refrain` eliminated most of them). The config flag defaults **on**, given those combined results, but this remains an honest spike: the lexicon is small and non-exhaustive by design, and a still-known false-positive category exists (a genuine negation phrase that reads as a stylistic near-duplicate, e.g. "do not overboard" against a same-meaning positive rewording). Turn it off (`same_polarity_contradiction_check = false`) if it proves noisy for a given store's vocabulary.

...
`possible_contradiction` (§M7c, experimental, config-gated by `same_polarity_contradiction_check`, default on) means a near-duplicate **learning** pair's wording shows a narrow antonym/negation asymmetry — see §7 below for the full heuristic and its calibration.
...
- **Conflict:** `capture` surfaces **cross-polarity** contradictions as its `conflict` status, resolved with the user via `revise`. A conflict is flagged when a new entry is highly similar to an existing opposite-polarity entry **and** one side *forbids* what the other *affirms* — decided by a **prohibition heuristic** on the issue text (`never`/`don't`/`avoid`/…). An aligned `Always …` mandate, or an aligned *negative* learning ("prefer avoiding X" next to "never X"), is therefore **not** flagged. Both directions are covered: new learning vs. existing issue (→ `remove`/`demote` the issue) and new issue vs. existing learning (→ `weaken`/`remove`/`promote` the learning), so `recall` can't later return both "do X" and "never do X". **`LEARNINGS`↔`LEARNINGS` contradictions are still *not* comprehensively auto-detected** — embeddings can't reliably separate a contradiction ("left-align") from a near-duplicate ("right-align") over the general case — so beyond the narrow check below, they are handled when the user explicitly routes the correction through `revise`/`weaken`. Detection is best-effort over the same-scope embedding neighbourhood already scanned for dedup — no separate subsystem, and a **heuristic** (prose polarity isn't perfectly recoverable from embeddings).
  - **§M7c — a narrow, additional same-polarity check (SPIKE, config-gated: `same_polarity_contradiction_check`).** On top of the embedding-similarity dedup match, `capture` runs a small, hand-picked, **explicitly non-exhaustive** antonym/negation lexicon (`_ANTONYM_PAIRS`/`_NEGATION_ASYMMETRY` in `server.py`: alignment, color temperature, spacing, orientation, width, plus a handful of negation markers) over the candidate's and the existing entry's combined title+body. It flags an **asymmetry** — one side says "left", the other "right"; one side is negated ("never"/"avoid"/"don't"/"refrain"), the other isn't — and returns `possible_contradiction` (the existing entry's id and current wording as `existing_body`, the candidate's `candidate_body`, and a short explanatory `note`) **instead of reinforcing**. Nothing is written in either case; it is a signal only, the same-polarity sibling of the `conflict` status above, and (like an unresolved `conflict`) an unresolved `possible_contradiction` surfaces as `compact`'s `conflict_residue` finding so it doesn't sit invisible forever. A pair that mentions **both** sides of a lexicon entry (e.g. "left and right padding" in both texts) is symmetric and is deliberately **not** flagged. **Argument-reversible pairs are deliberately excluded or restricted** — "before"/"after", "above"/"below", and "larger"/"smaller" were in an earlier version and were dropped entirely, since a genuine paraphrase can reverse both a relation/comparison AND its arguments ("Put the totals above the notes." ≡ "Put the notes below the totals."; "Make headings larger than body text." ≡ "Make body text smaller than headings.") in a way this word-presence check can't tell apart from a real flip without argument-aware parsing (out of scope for a lexical heuristic); "left"/"right" has the same relational risk ("put the legend left of the plot" ≡ "put the plot right of the legend") but is kept, restricted to a TIGHT alignment-compound match ("left-align"/"align ... left" as an immediate phrase, not merely co-occurring with "align" anywhere in the same sentence — the spec's own worked example, "right-align"/"left-align", stays covered; a spelled-out "align the columns to the left" is a known, accepted recall loss). The antonym and negation signals are reconciled with each other, scoped to the specific CLAUSE each side matched (splitting on sentence boundaries, semicolons, and a comma before a contrastive conjunction — not just sentences, since two clauses in one sentence can carry unrelated meanings): when the clause containing EITHER side's matched antonym word is negated, the pair is treated as an explained cancellation rather than a flip — covering a differing-negation cancellation ("Avoid a dark background." affirms "Use a light background."), a both-negated case that can also agree ("avoid wide tables; keep a medium width" and "avoid narrow tables; keep a medium width" both funnel to the stated target), and a same-sentence-different-clause case ("Avoid a dark background, but use wide margins." vs. "Use a light background, but use narrow margins." — the background clause cancels, the unrelated margins clause still fires) — while a SEPARATE, unrelated negation difference elsewhere in the body (outside the cancelled clauses) still surfaces via the plain negation-asymmetry fallback, so one legitimate cancellation can't mask a genuinely different flip nearby. Resolution is via `revise` on the flagged `entry_id`: `action:"reinforce"` with no `body` if it genuinely was a duplicate (bumps recurrence without re-running this check); `action:"remove"` (NOT `"weaken"`, which only lowers recurrence and typically leaves the entry indexed, so a fresh capture would flag again) followed by a fresh `capture` of the corrected wording if it was a real contradiction — NOT a reworded `reinforce`, which would keep the old entry's accrued recurrence and could push a single opposite observation straight to the promotion threshold. **The calling agent MUST confirm with the user before calling `remove`** — this is a heuristic signal, not a user decision, and `revise(action="remove")` on a learning is dial-governed (it deletes immediately outside supervised mode, unlike an issue contradiction, which always confirms regardless of mode); resolving a `possible_contradiction` by silently deleting an accrued preference would undercut the entire "signal only, human decides" premise this status exists for. Re-`capture`-ing the same body is a no-op against this deterministic check, not a resolution path. This is validated by a small hand-labeled set (`harness/calibration/labels.yaml`'s `same_polarity_contradiction` section, scored by `harness/calibrate.py`'s `calibrate_same_polarity_contradiction`): **1.0 precision / 1.0 recall** on that labeled set (6 contradiction pairs, 6 duplicate pairs including deliberately tricky same-lexicon-word and negation-cancellation near-misses), on both the hashing and sentence-transformers backends. That labeled set is, by construction, matched against the lexicon it tests, so it alone is not independent evidence of real-world precision — manual adversarial spot-checks against realistic (non-hand-picked) paraphrase pairs during this slice's build, PLUS **four rounds** of independent (Codex) review, each surfaced real precision/correctness gaps this slice's own labeled set didn't (negation word-list narrowing; negation-cancellation's clause-scoping and its unrelated-negation, both-negated, and same-sentence-different-clause refinements; the relational/comparative-antonym exclusions and the alignment-binding tightening; the missing `existing_body`; the `weaken`-doesn't-actually-remove and confirm-before-remove resolution-guidance bugs; and the residue-mining gap) — every one fixed and folded back into the code and this doc rather than hidden, which is itself informative: a hand-rolled lexical heuristic for this problem kept finding new edges under scrutiny even after each fix, so treat "1.0/1.0 on the labeled set" as necessary but not sufficient evidence of real-world precision. The config flag defaults **on** given the combined (if repeatedly-patched) evidence, but this remains an honest spike: the lexicon is small and non-exhaustive by design, and known false-positive/gap categories remain (a genuine negation phrase that reads as a stylistic near-duplicate, e.g. "do not overboard" against a same-meaning positive rewording; a semantic reconciliation via a third, shared explicit statement this lexical check has no way to see; any argument-reversible pattern not yet identified and excluded). Turn it off (`same_polarity_contradiction_check = false`) if it proves noisy for a given store's vocabulary — and see the M7c PR description for the explicit "is four rounds of finding-and-fixing itself evidence this should default off" question, flagged there rather than decided silently.

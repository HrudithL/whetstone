...
```json
{ "status": "committed" | "reinforced" | "noop" | "conflict" | "possible_contradiction" | "needs_confirmation", "entry_id": "L13", "recurrence": 1,
  "conflict": { "with_id": "I3", "explanation": "…" },
  "existing_body": "…the flagged entry's own current wording — title + body, since the heuristic compares both and a hand-edited title can carry the trigger word alone (possible_contradiction only, §M7c)…",
  "candidate_body": "…the new, uncommitted candidate's wording — title + body, same reason (possible_contradiction only, §M7c)…",
  "note": "…a short explanation of the detected antonym/negation asymmetry (possible_contradiction only, §M7c)…",
...
`possible_contradiction` (§M7c, experimental, config-gated by `same_polarity_contradiction_check`, **off by default**) means a near-duplicate **learning** pair's wording shows a narrow antonym/negation asymmetry — see §7 below for the full heuristic, its calibration, and why it defaults off.
...
- **Conflict:** `capture` surfaces **cross-polarity** contradictions as its `conflict` status, resolved with the user via `revise`. A conflict is flagged when a new entry is highly similar to an existing opposite-polarity entry **and** one side *forbids* what the other *affirms* — decided by a **prohibition heuristic** on the issue text (`never`/`don't`/`avoid`/…). An aligned `Always …` mandate, or an aligned *negative* learning ("prefer avoiding X" next to "never X"), is therefore **not** flagged. Both directions are covered: new learning vs. existing issue (→ `remove`/`demote` the issue) and new issue vs. existing learning (→ `weaken`/`remove`/`promote` the learning), so `recall` can't later return both "do X" and "never do X". **`LEARNINGS`↔`LEARNINGS` contradictions are still *not* comprehensively auto-detected** — embeddings can't reliably separate a contradiction ("left-align") from a near-duplicate ("right-align") over the general case — so beyond the narrow check below, they are handled when the user explicitly routes the correction through `revise`/`weaken`. Detection is best-effort over the same-scope embedding neighbourhood already scanned for dedup — no separate subsystem, and a **heuristic** (prose polarity isn't perfectly recoverable from embeddings).
  - **§M7c — a narrow, additional same-polarity check (SPIKE, config-gated: `same_polarity_contradiction_check`, OFF by default).** On top of the embedding-similarity dedup match, `capture` runs a small, hand-picked, **explicitly non-exhaustive** antonym/negation lexicon (`_ANTONYM_PAIRS`/`_NEGATION_ASYMMETRY` in `server.py`: alignment, color temperature, magnitude, orientation, plus a handful of negation markers — deliberately excludes argument-reversible pairs like wide/narrow, larger/smaller, more/less, and before/after, see the code comment for why) over the candidate's and the existing entry's combined title+body. It flags an **asymmetry** — one side says "left-align", the other "right-align"; one side is negated ("never"/"avoid"/"don't"/"refrain"), the other isn't — and returns `possible_contradiction` (the existing entry's id and current wording as `existing_body`, the candidate's `candidate_body`, and a short explanatory `note`) **instead of reinforcing**. Nothing is written in either case; it is a signal only, the same-polarity sibling of the `conflict` status above, and (like an unresolved `conflict`) an unresolved `possible_contradiction` surfaces as `compact`'s `conflict_residue` finding, with its candidate wording/note persisted onto the event so a later mining pass can still show what opposed the entry. A MANDATORY issue conflict always takes precedence over this softer signal — if the candidate also violates an existing issue, `capture` returns that `conflict` instead, never `possible_contradiction` — though this check, like every other conflict check `capture` runs, is LOCAL-store only; `capture` has never consulted the `__global__` store for cross-polarity conflicts (unlike `recall`, purpose-built for that in M5e/M7b), so a candidate that only conflicts with a *global*-origin issue is missed here exactly as it already was for the ordinary, non-M7c conflict path — a pre-existing boundary, not something this slice introduced or need solve.

    **Argument-reversible pairs are deliberately excluded or restricted**: "before"/"after", "above"/"below", "larger"/"smaller", and "more"/"less" were all in earlier versions and were dropped, since a genuine paraphrase can reverse both a relation/comparison AND its arguments ("Put the totals above the notes." ≡ "Put the notes below the totals."; "Use no more than two decimals." ≡ "Use two decimals or less.") in a way this word-presence check can't tell apart from a real flip without argument-aware parsing (out of scope for a lexical heuristic). "left"/"right" has the same relational risk ("put the legend left of the plot" ≡ "put the plot right of the legend") but is kept, restricted to a TIGHT alignment-compound match ("left-align"/"align ... left" as an immediate phrase — the spec's own worked example stays covered; a spelled-out "align the columns to the left" is a known, accepted recall loss).

    The antonym and negation signals are reconciled with each other, scoped to the specific CLAUSE each side matched (splitting on sentence boundaries, semicolons, and a comma before a contrastive conjunction): when the clause containing EITHER side's matched word is negated, the pair is an explained cancellation, not a flip — covering a differing-negation cancellation, a both-negated case that can still agree given a shared explicit target, and a same-sentence-different-clause case, while a separate, unrelated negation elsewhere in the body still surfaces. **Known, accepted limitations (documented, not fixed):** (1) ordinary coordinating conjunctions ("and", or "but" without a preceding comma) are NOT clause boundaries here — splitting on them would fix some missed contradictions but would just as often wrongly separate a negation from an item it still governs in the same clause ("avoid bold and flashy" splitting into "avoid bold" / "flashy", losing the negation on "flashy"), trading one false-negative class for a false-positive class with no clearly-better answer without real parsing. (2) Each side of a pair is looked up via its FIRST matching clause only — if the same antonym word appears in more than one clause of the same text, an earlier cancelled occurrence stops the pair from being re-checked against a later, genuinely uncancelled occurrence elsewhere in the same body. Fully handling either would mean real parsing or evaluating every occurrence independently, both a larger undertaking than this narrow lexical heuristic's scope. Left as documented gaps rather than patched with something likely to regress elsewhere.

    Resolution is via `revise` on the flagged `entry_id`: `action:"reinforce"` with no `body` if it genuinely was a duplicate; `action:"remove"` (NOT `"weaken"`, which typically leaves the entry indexed) followed by a fresh `capture` of the corrected wording if it was a real contradiction — NOT a reworded `reinforce`, which would keep the old entry's accrued recurrence. **The calling agent MUST confirm with the user before calling `remove`** — this is a heuristic signal, not a user decision, and `revise(action="remove")` on a learning is dial-governed (deletes immediately outside supervised mode, unlike an issue contradiction, which always confirms).

    **Calibration and review history.** This scores **1.0 precision / 1.0 recall** on a small hand-labeled set (`harness/calibration/labels.yaml`'s `same_polarity_contradiction` section, scored by `harness/calibrate.py`'s `calibrate_same_polarity_contradiction`: 6 contradiction pairs, 6 duplicate pairs including deliberately tricky near-misses), on both the hashing and sentence-transformers backends. That labeled set is, by construction, matched against the lexicon it tests, so it is not strong evidence on its own. During this slice's build, **six separate rounds** of independent (Codex) code review each found a real, distinct precision or correctness gap the labeled set didn't surface — spanning the negation word list, clause/occurrence scoping (five distinct refinements across four rounds), argument-reversible pair exclusions (four pairs identified across three rounds), a missing `existing_body`, two separate resolution-guidance bugs (`weaken` doesn't remove; removal isn't confirmation-gated), two residue-mining gaps (a missing status, then an event-ordering bug, then missing evidence fields), missing telemetry persistence, a global-store conflict-scope gap (documented as a pre-existing boundary, not fixed), a multi-occurrence antonym gap (documented, not fixed), and — the most serious — an ordering bug that let this status's own resolution guidance risk deleting a compatible, correct learning ahead of a mandatory issue conflict. Every fixable one was fixed and is recorded here rather than hidden; the two left unfixed are explicitly documented as known limitations above, not silently dropped. **Given that pattern — six consecutive rounds of real findings, including one genuine data-loss-risk interaction and two gaps with no clean fix available — `same_polarity_contradiction_check` defaults OFF.** The lexicon, clause-scoping, and resolution guidance are all real, tested, and available to opt into (`same_polarity_contradiction_check = true`), documented as experimental; the honest conclusion from this many rounds of scrutiny is that a hand-rolled lexical heuristic for this problem cannot yet be trusted as an on-by-default behavior, even though most individual bugs found were fixable. See the M7c PR description for the full round-by-round history.
